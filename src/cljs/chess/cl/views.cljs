...
(defn find-game-panel []
  [:div "test"])

(defn on-find-game-click [e]
  (.preventDefault e)
  (re-frame/dispatch [:find-game-click]))

(defn find-game-button []
...
(defn board-panel []
  (let [game (re-frame/subscribe [::subs/game])
        ...
        current-game (re-frame/subscribe [::subs/current-game])
        rotate? (and my-team (= my-team chess/BLACK))
        board (and current-game (:board @current-game))
        view-board (and board
                        (if rotate?
                          (chess/rotate-board board)
                          board))
        caps (and current-game (:captures @current-game))
...
(defn main-panel []
  (let [game (re-frame/subscribe [::subs/game])]
    (if @game
      [:div
       {:class "container mt-3 p-4 rounded"
